...
class StudentForm(forms.ModelForm):
    ...
    def clean_registration_number(self):
        ...
        if not registration_number.isalnum():
            raise forms.ValidationError(_("Registration number must be alphanumeric."))
    ...
    def clean_mother_mobile_number(self):
        phone_number = self.cleaned_data.get('mother_mobile_number')
        if phone_number and not phone_number.startswith('+255'):
            raise forms.ValidationError(_("Mother's mobile number must start with +255."))
        return phone_number
